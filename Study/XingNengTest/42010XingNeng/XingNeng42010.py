...
if __name__ == '__main__':
    ...
    response_synchronizeBasicDataForClient = req.visit("post", "http://172.16.9.2:42010/api/engine/4/scenes/10", json={
    ...
    response_synchronizeDigitalSupplyChainParameterForClient = \
        req.visit("post", "http://172.16.9.2:42010/api/engine/4/scenes/10", json={
    ...
    # 开启调用分析
    response_analyse_start = req.visit("post", 'http://172.16.9.2:42010/api/engine/call/4/10/analyse/start',
                                       headers=headers,
                                       verify=False)
    ...
    # 获取请求结果
    response_analyse_result = req.visit("get", 'http://172.16.9.2:42010/api/engine/call/4/10/analyse/result',
    ...
    # 结束分析
    response_analyse_end = requests.post('http://172.16.9.2:42010/api/engine/call/4/10/analyse/stop', headers=headers,
                                         verify=False)
    # 打印时长
    ...
    response_instanceCount = req.visit("get", 'http://172.16.9.2:42010/api/engine/4/scenes/10/instanceCount',
                                       headers=headers, verify=False)
    data = response_instanceCount.json()["data"]
    print(data)

    with open(f"./instanceCount-{datetime.now().strftime('%Y,%m,%d %H-%M-%S')}-XingNeng.csv", 'w', newline='',
              encoding='utf-8') as f:
        # csv.DictWriter()是一个用于将字典列表写入CSV文件的类。它接受两个参数：第一个参数是要写入的文件对象，第二个参数是字段名列表。
        # fieldnames=data[0].keys()表示从data列表的第一个元素（假设它是一个字典）中获取所有的键作为字段名。
        writer = csv.DictWriter(f, fieldnames=data[0].keys())
