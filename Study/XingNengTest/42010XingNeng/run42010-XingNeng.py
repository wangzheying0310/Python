...
headers = {
    ...
    'Authorization': 'eyJhbGciOiJIUzI1NiJ9.eyJOTyI6Ind6eSIsImlzcyI6IkdPT0RPIiwiSUQiOiIxMDEiLCJleHAiOjE2OTk2OTE2MjEsImlhdCI6MTY5OTUxMTYyMSwianRpIjoiVVNFUl9UT0tFTi4xMDEifQ.BYvgRDzfd4dKMHSXNg8aQSAG9KfKKPfKaI2JRR_GNUw',
    'Client-Identity': '8ff1592731cb56dc10926a2b2229999c',
    'Connection': 'keep-alive',
    'Content-Type': 'application/json',
    'Origin': 'http://172.16.9.2:42010',
    'Referer': 'http://172.16.9.2:42010/sceneManager',
...
# 发送请求
response_synchronizeBasicDataForClient = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/10', headers=headers, json=json_data)
# 记录同步基础数据结束时间
...
# 发送请求
response_synchronizeDigitalSupplyChainParameterForClient = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/10', headers=headers, json=json_data, verify=False)
# 记录同步供应链参数结束时间
...
# 开启调用分析
response_analyse_start = requests.post('http://172.16.9.2:42010/api/engine/call/4/10/analyse/start', headers=headers, verify=False)
# 发送请求
response = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/10', headers=headers, json=json_data)
# 记录同步数字供应链结束时间
...
# 获取请求结果
    response_analyse_result = requests.get('http://172.16.9.2:42010/api/engine/call/4/10/analyse/result',
                                           headers=headers, verify=False)
    print(response_analyse_result.json()["data"])
    json.dump(response_analyse_result.json(), f, ensure_ascii=False)
# 结束分析
response_analyse_end = requests.post('http://172.16.9.2:42010/api/engine/call/4/10/analyse/end', headers=headers,
                                     verify=False)
# 记录同步数字供应链结束系统时间
print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
json_data = {
...
# 发送请求
synchronizeSalesDataForClient_response = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/10', headers=headers, json=json_data)
# 记录销售需求结束时间
...
# 发送请求
capacityPlanningAlgorithm_response = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/10', headers=headers, json=json_data)
# 记录优化结束时间
