...
if __name__ == '__main__':
                              ...
                              'updateRows': [
                                  {
                                      'ktid': 0,
                                      'bomSource': 'KT',
                                      'materialDicSource': 'KT',
                                      'productionVersionSource': 'KT',
                                      'supplierInfoSource': 'KT',
                                      'unitConversionSource': 'KT',
                                  },
                              ],
                          },
                      ],
    ...
    preHandleDataForClient_response = \
        req.visit("post",
                  "http://172.16.5.10/apim/v2/scene/dataManager/Model03/companion-method",
                  json={
                      'type': 'DataManager',
                      'name': 'preHandleDataForClient',
                      'args': [],
                      'userNo': 'WZY',
                  }, headers=headers).json()["data"]
    ...
    if BOM_dict['total'] != 0 | MaterialDictionary_dict['total'] != 0 | ProductionVersion_dict['total'] != 0:
        print("未获取到数据，测试通过")
    else:
        print("测试未通过")
