...
if __name__ == '__main__':
    ...
    response_synchronizeBasicDataForClient = \
        req.visit("post", "http://172.16.5.10/apim/v2/scene/supplychain-planner/XingNeng1/companion-method", json={
    ...
    response_analyse_start = req.visit("post",
                                       'http://172.16.5.10/api/v1/performance/supplychain-planner/XingNeng1/startCollect',
                                       headers=headers,
                                       verify=False)
    ...
    response_analyse_result = req.visit("get",
                                        'http://172.16.5.10/api/v1/performance/supplychain-planner/XingNeng1/callAnalysis',
    ...
    # try:
    #     with open(f"./analyse_result-{datetime.now().strftime('%Y,%m,%d %H-%M-%S')}-510XingNeng2.csv", 'w', newline='',
    #               encoding='utf-8') as f:
    ...
    response_analyse_end = requests.post(
        'http://172.16.5.10/api/v1/performance/supplychain-planner/XingNeng1/endCollect', headers=headers,
        verify=False)
    # 打印时长
    ...
    with open(f"./data-{datetime.now().strftime('%Y,%m,%d %H-%M-%S')}-510XingNeng1.json", 'w') as f:
        # 获取请求结果
        response_instanceCount = req.visit("get",
                                           'http://172.16.5.10/api/v1/performance/supplychain-planner/XingNeng1/instanceData',
                                           headers=headers, verify=False)
        # print(response_instanceCount.json()["data"])
        # print(data)
        json.dump(response_instanceCount.json(), f, ensure_ascii=False)
