...
headers = {
    ...
    'Authorization': 'eyJhbGciOiJIUzI1NiJ9.eyJOTyI6Ind6eSIsImlzcyI6IkdPT0RPIiwiSUQiOiIxMDEiLCJleHAiOjE2OTk1ODcxOTMsImlhdCI6MTY5OTQwNzE5MywianRpIjoiVVNFUl9UT0tFTi4xMDEifQ.TiWEuIM_Ip9tyJi1q4A-8VgtaD7WwXav64-baA1WWXg',
...
# 发送请求
response_synchronizeBasicDataForClient = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/13', headers=headers, json=json_data, verify=False)
# 记录同步基础数据结束时间
...
# 发送请求
response_synchronizeDigitalSupplyChainParameterForClient = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/13', headers=headers, json=json_data, verify=False)
# 记录同步供应链参数结束时间
...
# 开启调用分析
response_analyse_start = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/13', headers=headers, json=json_data, verify=False)
# 记录同步数字供应链结束时间
...
# 发送请求
synchronizeSalesDataForClient_response = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/13', headers=headers, json=json_data)
# 记录销售需求结束时间
...
# 发送请求
capacityPlanningAlgorithm_response = requests.post('http://172.16.9.2:42010/api/engine/4/scenes/13', headers=headers, json=json_data)
# 记录优化结束时间
